...
import {HTTP_RES_MESSAGES,HTTP_CODE,TIMEOUT} from "../../../config/http.config"
...
const helper = {
  ...
  async postMockJson(url, params, isForm = false) {
    ...
    const data = (await
      fetch(`${mock_host}:${mock_port}${url}`, fetchConfig)
    )
    helper.removeProgress()
    return data.json()
  },
    ...
    if (error.response) {
      ...
      switch (status) {
        case HTTP_CODE['ERROR']:
          message.error(msg || '服务器发生错误!')
          break
        case HTTP_CODE['TIMEOUT']:
          message.error(msg || '请求超时!')
          break
        case HTTP_CODE['404']:
          message.error(msg || '接口404!')
          break
        case HTTP_CODE['TOKEN']:
          message.error(msg || '无权限!')
          break
        default:
          message.error(msg || '接口请求失败！')
      }
    } else {
      message.error(error && error.message ? error.message : "出错了!");
    }
    helper.removeProgress()
  },
  //全局处理错误
